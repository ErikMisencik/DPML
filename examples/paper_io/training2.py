...
from examples.paper_io.utils.plots import (
    plot_average_self_eliminations, plot_average_territory_increase, plot_average_trail, plot_cumulative_rewards, 
    plot_cumulative_self_eliminations, plot_epsilon_decay, plot_td_error,
...
if load_existing_model:
    ...
    epsilon_decay = 0.9998        # Standard decay rate for 10000 episodes
...
# NEW: Create the environment with default observation transformations.
# (Defaults are passed via trail_obs_representation and territory_obs_representation.)
env = PaperIoEnv(render=render_game, max_steps=steps_per_episode, num_players=num_agents, 
                 partial_observability=partial_observability)

# NEW: Set per-agent observation configurations based on algorithm type.
# For example:
# - Q-Learning and TD agents see trails as "border" and territories as "transformed".
# - SARSA and Monte Carlo agents see trails as "negative" and territories as "raw".
# - ActorCritic agents get a mixed configuration.
...
def save_training_info(file_path, num_episodes, steps_per_episode, agents, reward_config, loaded_q_paths, total_training_time):
    ...
    with open(file_path, 'w') as f:
        ...
        f.write("Objective: The agents aim to maximize territory gain.\n")
...
# ----------------------------------------------------------------------------------------
# TRAINING LOOP (with single-pass states & actions)
# ----------------------------------------------------------------------------------------
...
while episode_num < num_episodes:
    ...
    while not done:
        ...
        states_actions = [(agent.get_state(obs, i), agent.get_action(obs, i)) for i, agent in enumerate(agents)]
        ...
        next_states_actions = [(agent.get_state(next_obs, i), agent.get_action(next_obs, i)) for i, agent in enumerate(agents)]
    ...
    if ((episode_num + 1) % epsilon_reset_interval == 0) and epsilon_reset:
...
if any(len(lst) > 0 for lst in average_trail_by_agent_data):
    plot_average_trail(episodes, average_trail_by_agent_data, plots_folder, agent_names)
